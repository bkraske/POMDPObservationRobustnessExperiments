...
@with_kw struct DBatteryPOMDP <: POMDP{DRoverState, Int, Int}
    ...
    terminal_state::DRoverState         = DRoverState(GWPos(-1,-1), -1, -1, -1)
    ...
    stateindices::Vector{Int}       = cumprod([size[1], size[2], sand_types, length(damagedstates)-1])#max_fuel+1])
    candamage::Int                              = 0
...
function inbounds(m::DBatteryPOMDP, s::DRoverState)
    return 1 <= s[1] <= m.size[1] && 1 <= s[2] <= m.size[2]
end

function POMDPs.transition(mdp::DBatteryPOMDP, s::DRoverState, a::Int)
    if s.fuel == 0 || isterminal(mdp, s) || haskey(mdp.rewards, s.pos) || (s.pos ∈ mdp.sand && s.sand ∈ mdp.bad_sands)
        return Deterministic(DRoverState(GWPos(-1,-1), -1, -1, -1))
    end
    ...
    if mdp.candamage == 1 && a == 1
        return SparseCat([DRoverState(GWPos(new_x, new_y), new_fuel_1, sand, 1),
                          DRoverState(GWPos(new_x, new_y), new_fuel_1, sand, 2)],[0.5,0.5])
    end
    return Deterministic(DRoverState(GWPos(new_x, new_y), new_fuel_1, sand, damagedstate))
end

## States 

POMDPs.length(pomdp::DBatteryPOMDP) = pomdp.size[1]*pomdp.size[2]*pomdp.sand_types*(length(pomdp.damagedstates)-1)+1

function POMDPs.states(pomdp::DBatteryPOMDP)
    ss = vec([DRoverState(GWPos(x, y), z, v, b) for x in 1:pomdp.size[1], y in 1:pomdp.size[2], z in pomdp.max_fuel, v in 1:pomdp.sand_types, b in pomdp.damagedstates[2:end]])
    push!(ss, DRoverState(GWPos(-1,-1), -1, -1, -1))
end

function POMDPs.stateindex(pomdp::DBatteryPOMDP, s::DRoverState) 
   if isterminal(pomdp, s)
        return length(pomdp)
    end
    return s.pos[1] + pomdp.stateindices[1] * (s.pos[2] -1) + pomdp.stateindices[2]* (s.sand -1 ) + pomdp.stateindices[3]*(s.damaged) # + pomdp.stateindices[3] * (s.fuel)
end

function POMDPs.initialstate(pomdp::DBatteryPOMDP)
    ...
    for s in sands
        for d in pomdp.initialdamage
            push!(states, DRoverState(pos, pomdp.max_fuel, s, d))
        end
    end
...
POMDPs.actions(pomdp::DBatteryPOMDP) = 1:6
POMDPs.actionindex(mdp::DBatteryPOMDP, a::Int) = a

## Transition

POMDPs.isterminal(m::DBatteryPOMDP, s) = (s == m.terminal_state)

## Observation
POMDPs.observations(pomdp::DBatteryPOMDP) = 1:3


POMDPs.obsindex(::DBatteryPOMDP, o::Int) = o

function POMDPs.initialobs(pomdp::DBatteryPOMDP, s) 
    return observation(pomdp, s, :up, s)
end

function POMDPs.observation(pomdp::DBatteryPOMDP, a::Int, sp)
    if sp.fuel == -1
...
## Reward 
function POMDPs.reward(pomdp::DBatteryPOMDP, s, a)
    r = 0
    r+= 1<a<6 ? pomdp.per_step_rew : 0
    r+= a==4 ? -0.1 : 0
    r+= haskey(pomdp.rewards, s.pos) ? pomdp.rewards[s.pos] : 0
    r+= s.pos ∈ pomdp.obstacles ? -10.0 : 0
    return r
end
POMDPs.discount(pomdp::DBatteryPOMDP) = pomdp.discount
