...
# function stickyness_check(pomdp,pg,mc)
#     n_nodes = length(pg.nodes)
#     n_states = length(ordered_states(pomdp))
#     n_obs = length(ordered_observations(pomdp))

#     sa_map = rsa_mapping(n_nodes,n_states,pg) #MC Node to S,A
#     map = rsn_mapping(n_nodes,n_states)
#     n_nprime = Dict([(edge[1],pg.edges[edge]) => edge[2] for edge in keys(pg.edges)])

#     sa_o_dict = Dict()
#     sa_to_smc = Dict()

#     is_sticky = true
#     for smc in obs_start_idx(size(mc,1)):size(mc,1)
#         if !unreachable_check(mc,smc) #Pass on nodes which are not reachable under the problem def'n #CHECK ME?
#             println("======")
#             @info "MC State: $smc"
#             @info "State: $(ordered_states(pomdp)[map[smc][2]])"
#             sps = mc[:,smc]
#             n,s,_ = map[smc]
#             @info "Action, State: $(sa_map[smc])"
#             @info "Old Node: $n"
#             probs = zeros(n_obs) #preallocate
#             nzsps,nzvals = findnz(sps)
#             if nzsps != [smc]
#                 for i in eachindex(nzsps)
#                     prob = nzvals[i]
#                     spmc = nzsps[i]
#                     if prob > 0.0
#                         np,sp,_ = map[spmc]
#                         @assert s==sp
#                         @info "New Node: $np"
#                         o_idx = POMDPs.obsindex(pomdp,n_nprime[(n,np)]) #Issue, the same node pairs could have many observations
#                         @info "Observation: $(n_nprime[(n,np)])"
#                         probs[o_idx] += prob
#                     end
#                 end
#                 sa = sa_map[smc]
#                 # @info probs
#                 # @info sa
#                 haskey(sa_to_smc,sa) ? push!(sa_to_smc[sa],smc) : push!(sa_to_smc,sa => [smc])
#                 if !haskey(sa_o_dict,sa)
#                     push!(sa_o_dict,sa => probs)
#                 elseif !all(isapprox.(sa_o_dict[sa],probs;atol=maximum(eps.(probs))))
#                     @warn "Non-Sticky Matrix"
#                     @info "New Probs: $probs"
#                     @info "Old Probs: $(sa_o_dict[sa])"
#                     is_sticky = false
#                     return is_sticky, sa_to_smc, sa_o_dict
#                 end
#             end
#         end
#     end
